            ...
            if (token != null && token.Length > 0)
            {
                ...
                if (tenants.Any())
                {
                    ...
                    if (items.Any())
                    {
                        ...
                        JDE_Logs Log = new JDE_Logs { UserId = UserId, Description = "Archiwizacja pozycji zamówienia", TenantId = tenants.FirstOrDefault().TenantId, Timestamp = DateTime.Now, OldValue = new JavaScriptSerializer().Serialize(items.FirstOrDefault()), NewValue = "" };
                        db.JDE_Logs.Add(Log);
